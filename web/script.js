(() => {
  ...
  const renderRules = () => {
    $('rulesGrid').innerHTML = (window.MRX_RULES || []).map(r => `<article class="rule-card"><span class="number">${r.number || ''}</span><h3>${r.title || ''}</h3><p>${r.text || ''}</p></article>`).join('');
  };

  const renderTeam = () => {
    $('teamGrid').innerHTML = (window.MRX_TEAM || []).map(m => `<article class="team-card"><img src="${m.avatar || ''}" alt="${m.name || 'Staff'}"><h3>${m.name || ''}</h3><div class="role">${m.role || ''}</div><p>${m.about || ''}</p><div class="discord">${m.discord || ''}</div></article>`).join('');
  };

  const renderEvents = () => {
    $('eventsGrid').innerHTML = (window.MRX_EVENTS || []).map(e => `<article class="event-card">${e.image ? `<img class="event-image" src="${e.image}" alt="${e.title || 'Event'}">` : ''}<span class="status">${e.status || 'EVENT'}</span><h3>${e.title || ''}</h3><p>${e.description || ''}</p><div class="event-meta"><span>📅 ${e.date || ''}</span><span>🕒 ${e.time || ''}</span><span>📍 ${e.location || ''}</span><span>🏆 ${e.reward || ''}</span></div></article>`).join('');
  };
